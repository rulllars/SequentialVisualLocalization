...
namespace LocSLAM
{
    ...
    recursive_mutex MapPoint::mGlobalMutex;
                                                                          ...
                                                                          mpReplaced(static_cast<MapPoint *>(NULL)), mfMinDistance(0), mfMaxDistance(0), mpMap(pMap)
    {
        ...
        cv::Mat tmp = cv::Mat::zeros(3, 1, CV_32F);
        tmp.copyTo(mNormalVector);
    ...
    {
        unique_lock<recursive_mutex> lock2(mGlobalMutex);
    ...
    cv::Mat MapPoint::GetWorldPos()
    {
        ...
        cv::Size s = mWorldPos.size();

        if (s.height == 3 && s.width == 1)
    ...
    void MapPoint::ReportOutlierObservation(KeyFrame *pKF)
    {
        unique_lock<recursive_mutex> lock(mMutexFeatures);
        mnOutliers[pKF] += 1;
    }

    size_t MapPoint::GetNumOutlierObservation(KeyFrame *pKF)
    {
        return mnOutliers[pKF];
    }
